...
public final class Looper {
     ...
     * idle handlers and sync barriers are defined on the queue whereas preparing
    ...
    /**
     * If set, the looper will show a warning log if a message dispatch takes longer
     * than this.
     */
    private long mSlowDispatchThresholdMs;

    /**
     * If set, the looper will show a warning log if a message delivery (actual
     * delivery time - post time) takes longer than this.
     */
    ...
    private static void prepare(boolean quitAllowed) {
        ...
        // 创建Looper并设置给sThreadLocal，这样的get的时候就不会null了
    ...
    public static void loop() {
        ...
        for (;;) {
            // 这是一个死循环，从消息队列不断的获取消息
            Message msg = queue.next(); // might block
            if (msg == null) {
                // No message indicates that the message queue is quitting.
                // 由于刚创建的MessageQueue就开始轮询，队列里是没有消息的，等到Handler sendMessage enqueue后
                // 队列里才有消息
                return;
            }
            ...
            try {
                msg.target.dispatchMessage(msg); // 就是绑定Handler，
    ...
    private static boolean showSlowLog(long threshold, long measureStart, long measureEnd, String what, Message msg) {
        ...
        // For slow delivery, the current message isn't really important, but log it
        // anyway.
        Slog.w(TAG, "Slow " + what + " took " + actualTime + "ms " + Thread.currentThread().getName() + " h="
    ...
    /**
     * Control logging of messages as they are processed by this Looper. If enabled,
     * a log message will be written to <var>printer</var> at the beginning and
     * ending of each message dispatch, identifying the target Handler and message
     * contents.
     *
     * @param printer A Printer object that will receive log messages, or null to
    ...
    /**
     * Set a thresholds for slow dispatch/delivery log. {@hide}
     */
     ...
     * Quits the looper.
     * <p>
     * Causes the {@link #loop} method to terminate without processing any more
     ...
     * before the looper terminates. Consider using {@link #quitSafely} instead to
     ...
     * handled. However pending delayed messages with due times in the future will
     * not be delivered before the loop terminates.
     * </p>
     * <p>
     * Any attempt to post messages to the queue after the looper is asked to quit
     * will fail. For example, the {@link Handler#sendMessage(Message)} method will
